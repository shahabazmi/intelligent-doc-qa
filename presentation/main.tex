...
\begin{tikzpicture}[
  ...
  store/.style={rectangle,rounded corners=2pt,draw=black!55,minimum width=46mm,
                minimum height=8mm,inner sep=2pt,fill=yellow!12},
...
\node[store]  at (5.7, -4.9)  (storage)   {SQLite (\texttt{app.db})\quad+\quad Milvus Lite (HNSW, L2)};
...
% Write: embed → combined storage
\draw[arr] (embed.south) -- (storage.north west);

% Read: storage → retrieve, routed around the right side of Generator
\draw[rdarr] (storage.east) -- ++(2.6,0) -- ++(0,2.4) -- (retrieve.east);
\end{tikzpicture}
